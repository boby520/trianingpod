...
extension UIAlertController {
  ...
  func addColorPicker(color: UIColor = .black, title: String, selection: ColorPickerViewController.Selection?) {
    ...
    let storyboard = UIStoryboard(name: "color", bundle: nil)
    guard let vc = storyboard.instantiateViewController(withIdentifier: "ColorPicker") as? ColorPickerViewController else { return }
    set(vc: vc)
    
    set(title: title, font: .systemFont(ofSize: 17), color: color)
    vc.set(color: color) { new in
      color = new
      self.set(title: title, font: .systemFont(ofSize: 17), color: color)
    }
    //color.hexString
    color.hexString
